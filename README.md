...
| `V::load_partial(p,n)` `v.store_partial(p,n)` | the lanes of a set — `n` lanes, or any `LaneRange` / `LaneMask` — and **not one byte outside them**: the load and store for the tail of a buffer. `vmaskmov` on AVX, `{k}` on AVX-512, lane by lane elsewhere; a register the set does not reach is skipped. Tested against `PROT_NONE` guard pages |
