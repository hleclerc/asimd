 ...
 *
 * DELIBERATELY C, AND DELIBERATELY FREE OF THE STANDARD LIBRARY. `arm_neon.h` is provided by the
 * compiler, so clang can compile this for `armv7a-linux-gnueabihf` with NO cross toolchain and
 * NO sysroot -- which means the check runs on a developer's laptop and not only in CI. Pulling
 * in `<asimd/...>` would need libstdc++ headers for the target and put it back out of reach.
 *
...
int32x4_t   t_s32( const int   *p, int   *q, int32x4_t a, int32x4_t b )   { vst1q_s32(q, vld1q_s32(p)); return vaddq_s32(vsubq_s32(a,b), vdupq_n_s32(3)); }
uint32x4_t  t_u32( const unsigned *p, unsigned *q, uint32x4_t a, uint32x4_t b ) { vst1q_u32(q, vld1q_u32(p)); return vaddq_u32(vsubq_u32(a,b), vdupq_n_u32(3)); }
...
int64x2_t   t_s64( const long long *p, long long *q, int64x2_t a, int64x2_t b ) { vst1q_s64(q, vld1q_s64(p)); return vaddq_s64(vsubq_s64(a,b), vdupq_n_s64(3)); }
uint64x2_t  t_u64( const unsigned long long *p, unsigned long long *q, uint64x2_t a, uint64x2_t b ) { vst1q_u64(q, vld1q_u64(p)); return vaddq_u64(vsubq_u64(a,b), vdupq_n_u64(3)); }
int16x8_t   t_s16( const short *p, short *q, int16x8_t a, int16x8_t b ) { vst1q_s16(q, vld1q_s16(p)); return vaddq_s16(vsubq_s16(a,b), vdupq_n_s16(3)); }
uint16x8_t  t_u16( const unsigned short *p, unsigned short *q, uint16x8_t a, uint16x8_t b ) { vst1q_u16(q, vld1q_u16(p)); return vaddq_u16(vsubq_u16(a,b), vdupq_n_u16(3)); }
int8x16_t   t_s8 ( const signed char *p, signed char *q, int8x16_t a, int8x16_t b ) { vst1q_s8(q, vld1q_s8(p)); return vaddq_s8(vsubq_s8(a,b), vdupq_n_s8(3)); }
uint8x16_t  t_u8 ( const unsigned char *p, unsigned char *q, uint8x16_t a, uint8x16_t b ) { vst1q_u8(q, vld1q_u8(p)); return vaddq_u8(vsubq_u8(a,b), vdupq_n_u8(3)); }
...
uint32x4_t  k_u32( unsigned b ) { const unsigned s[4]={1,2,4,8}; uint32x4_t sel=vld1q_u32(s);
