...
//
// The table; the shapes are in `SimdOpsPlus_Shapes.h`, shared with x86. Laid out by feature
...
// exception -- so `SimdOpsPlus_Split.h` is load-bearing on this target in a way it is not on x86.
//
...
#define ASIMD_NEON_PLUS_CMP( COND, T, N, IS, SUF ) \
    ASIMD_PLUS_CMP( COND, cmp_gt, T, N, IS, REGISTER, vcgtq_##SUF( a.data.reg, b.data.reg ) ); \
    ASIMD_PLUS_CMP( COND, cmp_lt, T, N, IS, REGISTER, vcltq_##SUF( a.data.reg, b.data.reg ) ); \
    ASIMD_PLUS_CMP( COND, cmp_eq, T, N, IS, REGISTER, vceqq_##SUF( a.data.reg, b.data.reg ) ); \
    ASIMD_PLUS_CMP( COND, cmp_ge, T, N, IS, REGISTER, vcgeq_##SUF( a.data.reg, b.data.reg ) )
...
#define ASIMD_NEON_PLUS_SELECT( COND, T, N, IS, SUF ) \
    ASIMD_PLUS_SELECT( COND, T, N, IS, REGISTER, vbslq_##SUF( m.data.reg, a.data.reg, b.data.reg ) )
...
// `_mm_fmadd_ps( x, y, acc )` puts it last. Hence `ASIMD_PLUS_FMA_EXPR` rather than
// `ASIMD_PLUS_FMA` -- the operand order is part of the instruction, not of the operation.
ASIMD_PLUS_FMA_EXPR( NEON, FMA, FP32, 4, vfmaq_f32( c.data.reg, a.data.reg, b.data.reg ) );
...
#define ASIMD_NEON_PLUS_MLA( T, N, SUF ) \
    ASIMD_PLUS_FMA_EXPR( NEON, NEON, T, N, vmlaq_##SUF( c.data.reg, a.data.reg, b.data.reg ) )
...
// and keeps the flavour the vector types actually want. See `SimdOpsPlus_Split.h`.
ASIMD_PLUS_MASK_FROM_BITS( NEON, 4, 32, REGISTER, ( [ & ] {
...
ASIMD_PLUS_FMA_EXPR   ( ASIMD, FMA, FP64, 2, vfmaq_f64( c.data.reg, a.data.reg, b.data.reg ) );

ASIMD_PLUS_MASK_FROM_BITS( ASIMD, 2, 64, REGISTER, ( [ & ] {
...
#define ASIMD_NEON_PLUS_BCAST( T, N, SUF ) \
    ASIMD_PLUS_BCAST( ASIMD, T, N, vdupq_laneq_##SUF( v.data.reg, LANE & ( N - 1 ) ) )
...
// the `& 3` is not optional: `SimdOpsPlus_Split.h` hands a sub-permutation indices that point
