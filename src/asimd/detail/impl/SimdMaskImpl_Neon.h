...
// `to_bits` is a REDUCTION rather than a register move: see `SimdOpsPlus_Neon.h`. SVE is where
...
//
// The ABI rule of README section 3 applies to these unchanged: `SIMD_MASK_IMPL_REG_LARGE` gives
...
// struct SimdMaskImpl<...> -- one per lane width a comparison can produce at 128 bits.
SIMD_MASK_IMPL_REG_LARGE( NEON,  2, 64, uint64x2_t )
SIMD_MASK_IMPL_REG_LARGE( NEON,  4, 32, uint32x4_t )
SIMD_MASK_IMPL_REG_LARGE( NEON,  8, 16, uint16x8_t )
SIMD_MASK_IMPL_REG_LARGE( NEON, 16,  8, uint8x16_t )
...
SIMD_MASK_IMPL_REG_REDUCTION( ASIMD,  2, 64, any, vmaxvq_u32( vreinterpretq_u32_u64( mask.data.reg ) ) != 0 )
SIMD_MASK_IMPL_REG_REDUCTION( ASIMD,  4, 32, any, vmaxvq_u32( mask.data.reg ) != 0 )
SIMD_MASK_IMPL_REG_REDUCTION( ASIMD,  8, 16, any, vmaxvq_u16( mask.data.reg ) != 0 )
SIMD_MASK_IMPL_REG_REDUCTION( ASIMD, 16,  8, any, vmaxvq_u8 ( mask.data.reg ) != 0 )
...
SIMD_MASK_IMPL_REG_REDUCTION( ASIMD,  2, 64, all, vminvq_u32( vreinterpretq_u32_u64( mask.data.reg ) ) == std::uint32_t( 0xFFFFFFFFu ) )
SIMD_MASK_IMPL_REG_REDUCTION( ASIMD,  4, 32, all, vminvq_u32( mask.data.reg ) == std::uint32_t( 0xFFFFFFFFu ) )
SIMD_MASK_IMPL_REG_REDUCTION( ASIMD,  8, 16, all, vminvq_u16( mask.data.reg ) == std::uint16_t( 0xFFFFu ) )
SIMD_MASK_IMPL_REG_REDUCTION( ASIMD, 16,  8, all, vminvq_u8 ( mask.data.reg ) == std::uint8_t ( 0xFFu ) )
